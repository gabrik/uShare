...
bool isLiveMedia(const char *str){
     if (startsWith("mmsh://",str) || startsWith("rtsp://",str) || startsWith("http://",str) || startsWith("rtp://",str) )
         return true;
     return false;
}
...
static void metadata_add_file(struct ushare_t *ut, struct upnp_entry_t *entry, 
...
#endif
    {
        ...
        if(st_ptr!=NULL)
            ...
            child = upnp_entry_new(ut, name, file, entry, 0, false);
        if (child)
            upnp_entry_add_child(ut, entry, child);
    }
}

static void metadata_add_live(struct ushare_t *ut, struct upnp_entry_t *entry,  const char *file, const char *name){
    
    //char *real_file = get_ff_filename(file);
...
metadata_add_container(struct ushare_t *ut,
    ...
    if (isLiveMedia(container))  {
        ...
        return;
        //metadata_add_file(ut, entry, container, "live",NULL /*&st_live*/);
    }
    //
